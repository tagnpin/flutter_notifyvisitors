...
The SDK allows users to subscribe or unsubscribe from specific notification categories.

These categories are managed from the NVECTA Dashboard.
...
```dart
Notifyvisitors.shared.subscribePushCategory(preferenceList, isUnSubscribeFromAll);
```
...
|------------|------------|-------------|
| preferenceList | List<String> | List of notification categories |
| isUnSubscribeFromAll | bool | Subscribe or unsubscribe action |
...
```dart
var categoryInfo = ["offers", "promotions", "updates"];
Notifyvisitors.shared.pushPreferences(categoryInfo, false);
```

---

### Subscribe User To All Categories

```dart
Notifyvisitors.shared.pushPreferences(null, false);
```

---

### Unsubscribe User From Selected Categories

```dart
var categoryInfo = ["offers", "promotions"];
Notifyvisitors.shared.pushPreferences(categoryInfo, true);
```

---

### Unsubscribe User From All Categories

```dart
Notifyvisitors.shared.pushPreferences(null, true);
```
...
## (6) Send Push Payload to NVECTA SDK
...
## (7) [Push Notification Channels](/docs/android-push-notification-channels.md)
